...
const VisitSchedule: React.FC = () => {
    ...
    // Always calculate these values before any conditional returns
    const depotLocation = { lat: -7.8664161, lng: 110.1486773 }; // Same as in DriverView
    ...
    const stopsWithDistance = useMemo(() => {
        ...
        return todayRoute.stops.map((stop) => {
            ...
            return {
                ...stop,
                status: getVisitStatus(stop.visitId),
                distanceFromPrev: distance,
            };
        });
    }, [todayRoute, visits]); // Re-calculate if route or visit statuses change
    ...
    // Conditional rendering - order matters for hooks
    if (isLoadingRoutes || isLoadingVisits) {
        return <Card><p className="text-center p-4">Memuat jadwal...</p></Card>;
    }
    
    if (!todayRoute) {
        return <Card><p className="text-center p-4">Tidak ada rute kunjungan untuk Anda hari ini.</p></Card>;
    }
    
    if (allTasksCompleted) {
        return <Card className="text-center p-6"><h2 className="text-2xl font-bold text-green-600">Kerja Bagus!</h2><p>Semua kunjungan untuk hari ini telah selesai.</p></Card>;
    }

    if (!isStarted) {
        return (
            <Card className="text-center p-6">
                <h2 className="text-xl font-bold mb-4">Rencana Kunjungan Hari Ini</h2>
                <p className="text-lg mb-6">{todayRoute.stops.length} Toko</p>
                <button 
                    onClick={() => setIsStarted(true)} 
                    className="w-full bg-brand-primary text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2"
                >
                    <ICONS.navigation /> Mulai Rute
                </button>
            </Card>
        );
    }
    ...
    return (
        <div>
            ...
            {todayRoute && todayRoute.stops.length > 0 && (
                <SalesRouteMap stops={stopsWithDistance} depot={depotLocation} />
            )}
            
            <ol className="space-y-3 mt-4">
                {stopsWithDistance.map((stop, index) => (
                    <li key={stop.visitId}>
                        <Card className={`p-4 ${stop.status !== VisitStatus.UPCOMING ? 'opacity-60' : ''}`}>
                            <div className="flex items-start justify-between">
                                <div className="flex items-start gap-4">
                                    <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-white ${getStatusClass(stop.status).split(' ')[0]}`}>
                                        {index + 1}
                                    </span>
                                    <div>
                                        <p className="font-bold">{stop.storeName}</p>
                                        <p className="text-xs text-gray-500">{stop.address}</p>
                                        <p className="text-sm mt-1 font-semibold">{stop.purpose}</p>
                                        {stop.distanceFromPrev !== undefined && (
                                            <p className="text-xs text-brand-dark font-semibold mt-1">
                                                Jarak dari titik sebelumnya: {stop.distanceFromPrev.toFixed(2)} km
                                            </p>
                                        )}
                                    </div>
                                </div>
                                <a 
                                    href={`https://www.google.com/maps/search/?api=1&query=${stop.address}`} 
                                    target="_blank" 
                            ...
                            {stop.status === VisitStatus.UPCOMING && (
                                <div className="mt-4 grid grid-cols-2 gap-2">
                                    <button 
                                        onClick={() => handleAttemptSuccess(stop)} 
                                        className="bg-green-500 text-white font-semibold py-2 rounded-lg"
                                    >
                                        Selesai
                                    </button>
                                    <button 
                                        onClick={() => handleSkipVisit(stop)} 
                                        className="bg-yellow-500 text-white font-semibold py-2 rounded-lg"
                                    >
                                        Lewati
                                    </button>
                                </div>
            ...
            >
                ...
                <div className="flex justify-end gap-2 pt-4">
                    ...
                    <button 
                        ...
                        className="bg-brand-primary text-white py-2 px-4 rounded-lg"
                    >
                        Konfirmasi
                    </button>
                </div>
            ...
            <Modal 
                title={`Lewati Kunjungan: ${stopToSkip?.storeName}`} 
                isOpen={!!stopToSkip} 
            ...
            >
                <div className="space-y-4">
                    ...
                    <textarea 
                        ...
                        placeholder="Contoh: Toko tutup, pemilik tidak di tempat..."
                    ></textarea>
                    <div className="flex justify-end gap-2 pt-2">
                        <button 
                            onClick={() => setStopToSkip(null)} 
                            className="bg-gray-200 py-2 px-4 rounded-lg"
                        >
                            Batal
                        </button>
                        <button 
                            ...
                            disabled={!skipReason || updateVisitMutation.isPending} 
                            className="bg-brand-primary text-white py-2 px-4 rounded-lg disabled:bg-gray-400"
                        >
