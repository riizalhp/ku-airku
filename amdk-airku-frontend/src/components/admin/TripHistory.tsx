...
const DeliveryHistory: React.FC = () => {
    ...
    const paginatedTrips = useMemo(() => {
        ...
        return tripsWithDetails.slice(startIndex, endIndex);
    }, [tripsWithDetails, currentPage, itemsPerPage]);

    const totalPages = Math.ceil(tripsWithDetails.length / itemsPerPage);
    ...
    return (
        <div className="space-y-6">
            ...
            {!isLoading && tripsWithDetails.length > 0 && (
                <Pagination
                    currentPage={currentPage}
                    totalPages={totalPages}
                    onPageChange={setCurrentPage}
                    itemsPerPage={itemsPerPage}
                    totalItems={tripsWithDetails.length}
                />
            )}
