...
import { RoutePlan, User, Vehicle, RouteStop, SalesVisitRoutePlan, SalesVisitStop, VisitStatus, Coordinate } from '../../types';
...
const DeliveryHistory: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            {selectedTrip && (
                 <Modal title={`Detail Perjalanan: ${selectedTrip.driverName} (${selectedTrip.vehiclePlate})`} isOpen={!!selectedTrip} onClose={() => setSelectedTrip(null)} size="xl">
                    <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                       <RouteMap stops={selectedTrip.stops} depot={depotLocation} />
                       {sequencedStoreGroups.map((storeGroup, index) => {
                           const prevLocation = index === 0 ? depotLocation : sequencedStoreGroups[index - 1].location;
                           ...
                           return (
                            <div key={index} className="p-3 border rounded-lg bg-gray-50">
                                ...
                                <p className="text-xs text-gray-500 mb-2">{storeGroup.address}</p>
                                 {distance > 0 && (
                                    <div className="flex items-center gap-1.5 text-xs text-brand-secondary font-semibold mb-2">
...
const SalesVisitHistory: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            {selectedTrip && (
                 <Modal title={`Detail Kunjungan: ${selectedTrip.salesName}`} isOpen={!!selectedTrip} onClose={() => setSelectedTrip(null)} size="xl">
                    <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-2">
                        <RouteMap stops={selectedTrip.stops.map(s => ({ ...s, id: s.visitId }))} depot={depotLocation} />
                       {selectedTrip.stops.map((stop, index) => {
                           const prevLocation = index === 0 ? depotLocation : selectedTrip.stops[index-1].location;
