                                    ...
                                    <div key={trip.id} className="border-t pt-4">
                                        ...
                                        <div className="space-y-2 max-h-52 overflow-y-auto pr-2">
                                            {trip.stops.map((order, index) => {
                                                const prevLocation = index === 0 ? depotLocation : trip.stops[index-1].location;
                                                const distance = getDistance(prevLocation, order.location);
                                                return (
                                                    <div key={order.id} className="flex justify-between items-center text-sm p-3 bg-gray-50 rounded-lg">
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-gray-500">ID: {order.id.slice(0, 6)} | Unit: {order.load} {distance > 0 && `(+${distance.toFixed(1)} km)`}</p>
                                                        </div>
