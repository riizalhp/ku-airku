...
export const OrderManagement: React.FC = () => {
    ...
    const filteredOrders = useMemo(() => {
        ...
        if (isBatchMode) {
             ordersToDisplay = orders.filter(order => order.status === OrderStatus.PENDING);
        } else {
    ...
    }, [orders, filterStatus, searchTerm, isBatchMode, storeMap]);
    ...
    const selectableOrdersInBatchMode = useMemo(() => isBatchMode ? filteredOrders : [], [isBatchMode, filteredOrders]);
    
    const toggleBatchMode = () => {
        setIsBatchMode(prev => {
            if (!prev === false) { // Exiting batch mode
                setSelectedOrderIds([]);
                setBatchVehicleId('');
            }
            return !prev;
        ...
        <div className="p-8 space-y-6">
            ...
            <Card className="bg-blue-50 border border-blue-200">
                <div className="flex justify-between items-center">
                    <div>
                        <h2 className="text-lg font-bold text-brand-dark">Penugasan Pesanan Massal</h2>
                        <p className="text-sm text-gray-600">Gunakan mode ini untuk menugaskan beberapa pesanan 'Pending' ke satu armada.</p>
                    </div>
                    <button onClick={toggleBatchMode} className={`font-bold py-2 px-4 rounded-lg ${isBatchMode ? 'bg-red-500 text-white' : 'bg-brand-secondary text-white'}`}>
                        {isBatchMode ? 'Nonaktifkan Mode' : 'Aktifkan Mode'}
                    </button>
                </div>
                {isBatchMode && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end mt-4 pt-4 border-t">
                        <div>
                            <label className="text-sm font-medium">Langkah 1: Pilih Tanggal</label>
                            <input 
                                type="date" 
                        ...
                        <div>
                            <label className="text-sm font-medium">Langkah 2: Pilih Armada</label>
                            <select 
            ...
            <Card><div className={`grid grid-cols-1 md:grid-cols-2 gap-4 ${isBatchMode ? 'opacity-50' : ''}`}><div><label className="text-sm font-medium">Pencarian</label><input type="text" placeholder="Cari toko atau ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} disabled={isBatchMode} className="w-full p-2 border rounded-md mt-1"/></div><div><label className="text-sm font-medium">Status</label><select value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)} disabled={isBatchMode} className="w-full p-2 border rounded-md mt-1"><option value="all">Semua Status</option>{Object.values(OrderStatus).map(s => <option key={s} value={s}>{s}</option>)}</select></div></div></Card>
                        ...
                        <thead className="bg-gray-50 text-xs text-gray-700 uppercase">
                            <tr>
                                <th scope="col" className="px-4 py-3">
                                    <input type="checkbox" className="h-4 w-4 rounded" disabled={!isBatchMode || selectableOrdersInBatchMode.length === 0} checked={selectableOrdersInBatchMode.length > 0 && selectedOrderIds.length === selectableOrdersInBatchMode.length} onChange={handleSelectAll} aria-label="Select all orders"/>
                                </th>
                            ...
                            {isLoading ? (<tr><td colSpan={10} className="text-center p-6">Memuat pesanan...</td></tr>) : filteredOrders.map(order => (
                                <React.Fragment key={order.id}>
                                    <tr className={`bg-white border-b hover:bg-gray-50 ${order.priority ? 'bg-yellow-50' : ''}`}>
                                        <td className="px-4 py-3">{order.status === OrderStatus.PENDING && isBatchMode && (<input type="checkbox" className="h-4 w-4 rounded" checked={selectedOrderIds.includes(order.id)} onChange={() => handleSelectOrder(order.id)} aria-label={`Select order ${order.id}`}/>)}</td>
